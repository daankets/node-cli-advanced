...
class CommandArgument<T> implements IArgument<T> {
    ...
    isRequired(args?: Map<string, any>): boolean {
...
export class Command<R> {
    ...
    toString() {
        ...
        let maxNameLength = Command.getMaxStringLength(Object.keys(this.$params));
        let maxTypeLength = Command.getMaxStringLength(
            ...
            Array
                ...
                .map(param => {
                    return "\t--" + Command.rightPad(param.name, maxNameLength) +
                        "\t" + Command.rightPad(param.flag ? "Flag" : (param.parser?.name || "string") +
    ...
    parse(ignoreStrict = false, ...args: string[]): CommandInstance<R> {
        const resolvedArguments = new Map<string, any>();
        ...
        args.forEach((argument) => {
            if (argument.startsWith("--")) {
                // Named, full style
                let paramName = argument.substr(2);
                ...
                let shortCode = argument.substr(1);
        ...
        const missingRequiredFields = Array
            ...
            .filter(param => [undefined, null].includes(resolvedArguments.get(param.name)))
